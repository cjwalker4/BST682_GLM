...
$$log(\theta)=\prod\limits_{i=0}^{n}{ f(y_i; \theta)}$$
$$=\prod\limits_{i=0}^{n}{n_i \choose  {y_i}}\theta^{y_i}(1-\theta)^{n_i -y_i} $$
$$=\theta^{\sum{y_i}}*(1-\theta)^{\sum{n_i-y_i} }*x $$
$$Log(\theta)=\sum y_i * log(\theta) +\sum(n_i-y_i)*log(1-\theta)$$
$$\frac{d}{d\theta}Log{\theta} = \frac{\sum y_i}{\theta}-\frac{\sum n_i - y_i}{1-\theta}$$
$$0=\frac{\sum y_i}{\theta}-\frac{\sum n_i - y_i}{1-\theta}$$
$$\sum y_i=\theta\sum y_i -\theta \sum n_i - \theta \sum y_i$$
$$\theta = \frac{\sum y_i}{\sum n_i}$$
$$\hat{\theta} =\frac {363}{734}=0.49$$
